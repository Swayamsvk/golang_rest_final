...
func getPost(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		w.Write([]byte("ID could not be cnverted to an integer"))
		return
	}

	//error checking
	//dont use in real world projects
	if id >= len(posts) {
		w.WriteHeader(404)
		w.Write([]byte("No post found for specified ID"))
		return
	}
...
func updatePost(w http.ResponseWriter, r *http.Request) {
	...
	//get the vlue from the json body
